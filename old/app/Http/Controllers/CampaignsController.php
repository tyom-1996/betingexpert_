...
class CampaignsController extends Controller
{
	...
	public function view($id, $slug = null) {
		...
		if( str_slug( $response->title ) == '' ) {
			...
			$percentage = number_format($response->funds / $response->goal * 100, 2, '.', '');
		...
		$dirname=$_SERVER['DOCUMENT_ROOT'].'/public/imgs/'.$id;
		
		if (!file_exists($dirname)) {
			mkdir($dirname);
		}
		$imgs = scandir($dirname);
		unset($imgs[0]);
		unset($imgs[1]);
	...
	{
		$telegram = new Api('1271333006:AAGbIjIHAsITwMZ5JzQ_b6gk46VKbB3b2a4');
	...
	public function update($id){
    	...
    	$telegram = new Api('1271333006:AAGbIjIHAsITwMZ5JzQ_b6gk46VKbB3b2a4'); //Устанавливаем токен, полученный у BotFather
		$data = Campaigns::where('id', $this->request->id)
		->where('user_id', Auth::user()->id)
		->firstOrFail();
